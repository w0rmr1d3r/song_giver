<?php
	...
	*/
	...
	if($_POST)
	{
        ...
        $category = preg_replace('(/^[a-zA-Z0-9 "!?.-]+$/)', '', cleanInput($_POST['song-category-input']));
		...
		{
			require(__MODEL__.'DB.php');

            $DB = DataBase::getInstance();

            if($DB->validFileName($fileName))
            {
                $DB->insertRegisterData(new Song($title, $fileName, $artist, $album, $category));
